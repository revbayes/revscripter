<script>
    ...
    let allFilteredChecked = $derived(
        filteredTaxa.length > 0 && filteredTaxa.every((taxon) => checkedTaxa.has(taxon))
    );
    ...
    function saveEdit() {
        ...
        const invalidTaxon = uniqueTaxa.find((taxon) => !analysisState.taxa.includes(taxon));
        if (invalidTaxon) {
    ...
    <div style="display: flex; gap: 2rem; margin-top: 1rem; align-items: start;">
                            ...
                            <tr>
                                <td>
                                    <input
                                        ...
                                        onchange={(e) => toggleTaxon(taxon, e.currentTarget.checked)}
                                    />
                                </td>
                                <td>{taxon}</td>
                                {#if taxaState.ageMode === 'range'}
        ...
        <div style="flex: 1;">
            <div style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem;">
                <input type="text" placeholder="Group Name" bind:value={groupName} style="flex: 1;" />
                ...
                <table>
                    ...
                    <tbody>
                        {#if taxaState.groups.length === 0}
                            <tr>
                                <td colspan="4">No group is created...</td>
                            </tr>
                        {:else}
                            {#each taxaState.groups as group, i}
                                <tr>
                                    <td>{group.name}</td>
                                    <td><input type="checkbox" bind:checked={group.monophyletic} /></td>
                                    <td><input type="checkbox" bind:checked={group.monitorAge} /></td>
                                    <td>
                                        <button onclick={() => openEdit(i)} title="Edit">✎</button>
                                    </td>
                                </tr>
                            {/each}
                        {/if}
...
<dialog bind:this={dialogEl} class="modal-box" onclick={handleDialogClick} onclose={closeEdit}>
    {#if editingIndex !== null}
        {@const group = taxaState.groups[editingIndex]}
